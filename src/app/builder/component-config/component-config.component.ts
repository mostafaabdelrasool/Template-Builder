...
export class ComponentConfigComponent implements OnInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<ComponentConfigComponent>,
  ...
  save() {
    if (this.codeModel.value) {
      ...
      const data = (({ id, name, dataStructure, featureId }) => ({
        id,
        name,
        dataStructure,
        featureId,
      ...
      this.builderService
        .saveDataStructure(<Form>data)
        .subscribe((x) => this.dialogRef.close(this.codeModel.value));
    }
  }
  close() {
