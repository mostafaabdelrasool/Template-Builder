...
export class DataMapperSettingComponent implements OnInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<DataMapperSettingComponent>,
  ...
  ngOnInit() {
    ...
    if (this.data.source === "currentItem" && this.data.field.dataSource) {
      this.sourceProps = (<FieldDataSource>(
        this.data.field.dataSource
      )).dataStructure.map((x) => x.name);
    }
  }
  add() {
  ...
  save() {
    this.dialogRef.close({
      mapper: this.propMapper,
      code: this.currentCodeModel.value,
    });
  }
}
